...
@section('title_page', 'About Hero')
@section('desc_page', 'Edit Content About Hero')
@section('content')
<div class="card">
    <div class="card-header">
        <h4 class="card-title"></h4>
        <button type="submit" style="float: right" class="btn btn-primary btn-add-kategori">Add About Heroes</button>
    </div>
    <div class="card-body">
        @include('components.table-pagination', ['table' => 'about_heroes' , 'url' => '/api/v1/admin/abouthero' , 'headers' => [
            "Heading",
            "Description",
            "Left Image",
            "Right Image",
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel4">Add About-Hero</h4>
                ...
                <form id="form-add-kategori">
                    ...
                    <table class="table table-striped table-komponent after-loading">
                        <tbody>
                            ...
                            <tr>
                                <th scope="row">Left Image</th>
                                <td> 
                                    <input type="file" name="repeater[0][left_image][]" class="form-control" accept="image/*" required > 
                                </td>
                            </tr>
                            <tr>
                                <th scope="row">Right Image</th>
                                <td> 
                                    <input type="file" name="repeater[0][right_image][]" class="form-control" accept="image/*" required > 
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
<script>
    $(document).ready(function() {
        GetData(req,"about_heroes", formatabouthero);
    });

    function formatabouthero(data) {
        var result = "";
        $.each(data, function(index, data) {
            ...
            result += `
                <tr>
                    ...
                    <td>${!empty(data.left_image) ? `<a href="#" class="openPopup" link="${data.left_image[0].url}">View File</a> `: "-"}</td>
                    <td>${!empty(data.right_image) ? `<a href="#" class="openPopup" link="${data.right_image[0].url}">View File</a> `: "-"}</td>
                    <td>
    ...
    $("#form-add-kategori").on('submit', function(e) {
        e.preventDefault();
        const url = `${baseUrl}/api/v1/admin/addOrUpdateAboutHero`;
    ...
    $(document).on('click', '.btn-detail', function() {
        ...
        ajaxData(`${baseUrl}/api/v1/admin/spaceshero`, 'GET', {
            "id" : $(this).data('id')
        }, function(resp) {
    ...
    $(document).on('click', '.btn-edit', function() {
        ...
        ajaxData(`${baseUrl}/api/v1/admin/abouthero`, 'GET', {
            "id" : $(this).data('id')
        }, function(resp) {
