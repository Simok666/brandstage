...
@section('title', 'Spaces Hero')
@section('title_page', 'Spaces Hero')
@section('desc_page', 'Edit Content Spaces Hero')
@section('content')
<div class="card">
    <div class="card-header">
        <h4 class="card-title"></h4>
        <button type="submit" style="float: right" class="btn btn-primary btn-add-kategori">Add Spaces Heroes</button>
    </div>
    <div class="card-body">
        @include('components.table-pagination', ['table' => 'space_heroes' , 'url' => '/api/v1/admin/spaceshero' , 'headers' => [
            "Heading",
            "Description",
            "Button Title",
            "Button Link",
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel4">Add Spaces-Hero</h4>
                    ...
                    <table class="table table-striped table-komponent after-loading">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td> 
                                    <input type="file" name="repeater[0][hero_image][]" class="form-control" accept="image/*" required > 
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
<script>
    $(document).ready(function() {
        GetData(req,"space_heroes", formatspacehero);
    });

    function formatspacehero(data) {
        var result = "";
        $.each(data, function(index, data) {
            ...
            result += `
                <tr>
                    ...
                    <td>${!empty(data.hero_image) ? `<a href="#" class="openPopup" link="${data.hero_image[0].url}">View File</a> `: "-"}</td>
                    <td>
    ...
    $("#form-add-kategori").on('submit', function(e) {
        e.preventDefault();
        const url = `${baseUrl}/api/v1/admin/addOrUpdateSpacesHero`;
